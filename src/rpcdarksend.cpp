...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <sibcoinaddress> <amount>\n"
            "sibcoinaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <sibcoinaddress> <amount>\n"
            "sibcoinaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in sibcoin.conf\n"
                ...
                "  vote-many    - Vote on a Sibcoin initiative\n"
                "  vote         - Vote on a Sibcoin initiative\n"
                );
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "protocol" && strMode != "full" && strMode != "votes" && strMode != "donation" && strMode != "pose"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Sibcoin initiative (can be additionally filtered, partial match)\n"
                );
    }
