...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                strErr = "Invalid Sibcoin address in masternode.conf line: " + line;
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 1945) {
                strErr = "Invalid port detected in masternode.conf: " + line + " (must be 1945 for mainnet)";
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 1945) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (1945 must be only on mainnet)";
            streamConfig.close();
            return false;
        }
